defmodule Jido.Code.Skill.Observability.TestActions.HookSignal do
end

defmodule Jido.Code.Skill.Observability.TestSkills.ValidLifecycleHook do
...
defmodule Jido.Code.Skill.Observability.TestSkills.InvalidLifecycleHook do
  def skill_metadata do
    %{
      hooks: %{
        pre: %{
...
defmodule Jido.Code.Skill.Observability.TestSkills.InheritedLifecycleHook do
  def skill_metadata do
    %{
      hooks: %{
        pre: %{
...
defmodule Jido.Code.Skill.Observability.TestSkills.ExplicitLifecycleHook do
  def skill_metadata do
    %{
      hooks: %{
        pre: %{
...
defmodule Jido.Code.Skill.Observability.TestSkills.InheritGlobalSignalTypeLifecycleHook do
  def skill_metadata do
    %{
      hooks: %{
        pre: %{
...
defmodule Jido.Code.Skill.Observability.LifecycleSubscriberTestRegistry do
  use GenServer
...
defmodule Jido.Code.Skill.Observability.LifecycleSubscriberNthLookupVia do
  def whereis_name({registry, lookup_plan}) do
    ...
    if fail_lookup? do
      case fail_mode do
        ...
        :exit ->
          exit(:simulated_lifecycle_registry_lookup_exit)
...
defmodule Jido.Code.Skill.Observability.SkillLifecycleSubscriberTest do
  use ExUnit.Case, async: false

  alias Jido.Code.Skill.Observability.LifecycleSubscriberTestRegistry
  ...
  test "migrates lifecycle subscriptions to refreshed signal bus after registry settings reload" do
    ...
    assert_eventually(fn ->
      :ok =
        publish_lifecycle_signal(old_bus_name, "skill.pre", "/hooks/skill/pre", "before-reload")
  ...
  test "keeps cached lifecycle bus when bus_name lookup is invalid and migrates after recovery" do
    old_bus_name = "bus_#{System.unique_integer([:positive])}"
    reloaded_bus_name = "bus_#{System.unique_integer([:positive])}"
    ...
    assert_eventually(fn ->
      :ok =
        publish_lifecycle_signal(old_bus_name, "skill.pre", "/hooks/skill/pre", "before-refresh")

      receive do
        {:telemetry, @telemetry_event, %{count: 1}, metadata} ->
          metadata.type == "skill.pre" and metadata.skill_name == "before-refresh" and
            metadata.bus == old_bus_name
      after
        80 ->
          false
    ...
    assert_eventually(fn ->
      :ok =
        publish_lifecycle_signal(old_bus_name, "skill.pre", "/hooks/skill/pre", "after-invalid")

      receive do
        {:telemetry, @telemetry_event, %{count: 1}, metadata} ->
          metadata.type == "skill.pre" and metadata.skill_name == "after-invalid" and
            metadata.bus == old_bus_name
      after
        80 ->
          false
    ...
    drain_telemetry_messages()
    assert :ok = LifecycleSubscriberTestRegistry.set_bus_name_error(registry, nil)
    assert :ok = publish_registry_update_signal(old_bus_name)
    ...
    assert_eventually(fn ->
      :ok =
        publish_lifecycle_signal(
          ...
          "skill.pre",
          "/hooks/skill/pre",
          "after-recovery-new-bus"
        )

      receive do
        {:telemetry, @telemetry_event, %{count: 1}, metadata} ->
          metadata.type == "skill.pre" and metadata.skill_name == "after-recovery-new-bus" and
            metadata.bus == reloaded_bus_name
      after
        80 ->
          false
  ...
  test "keeps cached lifecycle bus when startup bus_name lookup stays invalid across refreshes and migrates after recovery" do
    old_bus_name = "bus_#{System.unique_integer([:positive])}"
    reloaded_bus_name = "bus_#{System.unique_integer([:positive])}"
    ...
    assert_eventually(fn ->
      :ok =
        publish_lifecycle_signal(
          ...
          "skill.pre",
          "/hooks/skill/pre",
          "before-startup-invalid-repeated-refresh"
        )

      receive do
        {:telemetry, @telemetry_event, %{count: 1}, metadata} ->
          metadata.type == "skill.pre" and
            metadata.skill_name == "before-startup-invalid-repeated-refresh" and
            metadata.bus == old_bus_name
      after
        80 ->
          false
    ...
    assert_eventually(fn ->
      :ok =
        publish_lifecycle_signal(
          ...
          "skill.pre",
          "/hooks/skill/pre",
          "after-startup-invalid-repeated-refresh-old-bus"
        )

      receive do
        {:telemetry, @telemetry_event, %{count: 1}, metadata} ->
          metadata.type == "skill.pre" and
            metadata.skill_name == "after-startup-invalid-repeated-refresh-old-bus" and
            metadata.bus == old_bus_name
      after
        80 ->
          false
      ...
      fn ->
        :ok =
          publish_lifecycle_signal(
            ...
            "skill.pre",
            "/hooks/skill/pre",
            "after-startup-invalid-repeated-refresh-new-bus"
          )
      end,
      6,
      80
      ...
      fn ->
        :ok =
          publish_lifecycle_signal(
            ...
            "skill.pre",
            "/hooks/skill/pre",
            "after-startup-invalid-recovery-old-bus"
          )
      end,
      6,
      80
    ...
    assert_eventually(fn ->
      :ok =
        publish_lifecycle_signal(
          ...
          "skill.pre",
          "/hooks/skill/pre",
          "after-startup-invalid-recovery-new-bus"
        )

      receive do
        {:telemetry, @telemetry_event, %{count: 1}, metadata} ->
          metadata.type == "skill.pre" and
            metadata.skill_name == "after-startup-invalid-recovery-new-bus" and
            metadata.bus == reloaded_bus_name
      after
        80 ->
          false
  ...
  test "keeps cached lifecycle bus when bus_name lookup stays invalid during refresh and migrates after recovery" do
    old_bus_name = "bus_#{System.unique_integer([:positive])}"
    reloaded_bus_name = "bus_#{System.unique_integer([:positive])}"
    ...
    assert_eventually(fn ->
      :ok =
        publish_lifecycle_signal(
          ...
          "skill.pre",
          "/hooks/skill/pre",
          "before-repeated-invalid-refresh"
        )

      receive do
        {:telemetry, @telemetry_event, %{count: 1}, metadata} ->
          metadata.type == "skill.pre" and
            metadata.skill_name == "before-repeated-invalid-refresh" and
            metadata.bus == old_bus_name
      after
        80 ->
          false
    ...
    assert :ok =
             LifecycleSubscriberTestRegistry.set_bus_name_error(
               registry,
    ...
    assert_eventually(fn ->
      :ok =
        publish_lifecycle_signal(
          ...
          "skill.pre",
          "/hooks/skill/pre",
          "after-repeated-invalid-refresh-old-bus"
        )

      receive do
        {:telemetry, @telemetry_event, %{count: 1}, metadata} ->
          metadata.type == "skill.pre" and
            metadata.skill_name == "after-repeated-invalid-refresh-old-bus" and
            metadata.bus == old_bus_name
      after
        80 ->
          false
      ...
      fn ->
        :ok =
          publish_lifecycle_signal(
            ...
            "skill.pre",
            "/hooks/skill/pre",
            "after-repeated-invalid-refresh-new-bus"
          )
      end,
      6,
      80
      ...
      fn ->
        :ok =
          publish_lifecycle_signal(
            ...
            "skill.pre",
            "/hooks/skill/pre",
            "after-repeated-invalid-recovery-old-bus"
          )
      end,
      6,
      80
    ...
    assert_eventually(fn ->
      :ok =
        publish_lifecycle_signal(
          ...
          "skill.pre",
          "/hooks/skill/pre",
          "after-repeated-invalid-recovery-new-bus"
        )

      receive do
        {:telemetry, @telemetry_event, %{count: 1}, metadata} ->
          metadata.type == "skill.pre" and
            metadata.skill_name == "after-repeated-invalid-recovery-new-bus" and
            metadata.bus == reloaded_bus_name
      after
        80 ->
          false
  ...
  test "keeps configured lifecycle bus when bus_name lookup raises call exceptions during startup and migrates after recovery" do
    old_bus_name = "bus_#{System.unique_integer([:positive])}"
    reloaded_bus_name = "bus_#{System.unique_integer([:positive])}"
    ...
    lookup_plan =
      start_supervised!(
        {Agent,
         fn ->
           %{count: 0, fail_on: MapSet.new([3])}
         end}
      )

    exception_registry =
      {:via, Jido.Code.Skill.Observability.LifecycleSubscriberNthLookupVia,
       {registry, lookup_plan}}
    ...
    assert_eventually(fn ->
      :ok =
        publish_lifecycle_signal(
          ...
          "skill.pre",
          "/hooks/skill/pre",
          "startup-call-exception-old-bus"
        )

      receive do
        {:telemetry, @telemetry_event, %{count: 1}, metadata} ->
          metadata.type == "skill.pre" and metadata.skill_name == "startup-call-exception-old-bus" and
            metadata.bus == old_bus_name
      after
        80 ->
          false
      ...
      fn ->
        :ok =
          publish_lifecycle_signal(
            ...
            "skill.pre",
            "/hooks/skill/pre",
            "startup-call-exception-recovery-old-bus"
          )
      end,
      6,
      80
    ...
    assert_eventually(fn ->
      :ok =
        publish_lifecycle_signal(
          ...
          "skill.pre",
          "/hooks/skill/pre",
          "startup-call-exception-recovery-new-bus"
        )

      receive do
        {:telemetry, @telemetry_event, %{count: 1}, metadata} ->
          metadata.type == "skill.pre" and
            metadata.skill_name == "startup-call-exception-recovery-new-bus" and
            metadata.bus == reloaded_bus_name
      after
        80 ->
          false
  ...
  test "keeps configured lifecycle bus when startup bus_name lookup keeps raising call exceptions across refreshes and migrates after recovery" do
    old_bus_name = "bus_#{System.unique_integer([:positive])}"
    reloaded_bus_name = "bus_#{System.unique_integer([:positive])}"
    ...
    lookup_plan =
      start_supervised!(
        {Agent,
         fn ->
           %{count: 0, fail_on: MapSet.new([3, 6, 9])}
         end}
      )
    ...
    assert_eventually(fn ->
      :ok =
        publish_lifecycle_signal(
          ...
          "skill.pre",
          "/hooks/skill/pre",
          "startup-call-exception-repeated-old-bus"
        )

      receive do
        {:telemetry, @telemetry_event, %{count: 1}, metadata} ->
          metadata.type == "skill.pre" and
            metadata.skill_name == "startup-call-exception-repeated-old-bus" and
            metadata.bus == old_bus_name
      after
        80 ->
          false
    ...
    assert_eventually(fn ->
      :ok =
        publish_lifecycle_signal(
          ...
          "skill.pre",
          "/hooks/skill/pre",
          "startup-call-exception-repeated-refresh-old-bus"
        )

      receive do
        {:telemetry, @telemetry_event, %{count: 1}, metadata} ->
          metadata.type == "skill.pre" and
            metadata.skill_name == "startup-call-exception-repeated-refresh-old-bus" and
            metadata.bus == old_bus_name
      after
        80 ->
          false
      ...
      fn ->
        :ok =
          publish_lifecycle_signal(
            ...
            "skill.pre",
            "/hooks/skill/pre",
            "startup-call-exception-repeated-refresh-new-bus"
          )
      end,
      6,
      80
      ...
      fn ->
        :ok =
          publish_lifecycle_signal(
            ...
            "skill.pre",
            "/hooks/skill/pre",
            "startup-call-exception-repeated-recovery-old-bus"
          )
      end,
      6,
      80
    ...
    assert_eventually(fn ->
      :ok =
        publish_lifecycle_signal(
          ...
          "skill.pre",
          "/hooks/skill/pre",
          "startup-call-exception-repeated-recovery-new-bus"
        )

      receive do
        {:telemetry, @telemetry_event, %{count: 1}, metadata} ->
          metadata.type == "skill.pre" and
            metadata.skill_name == "startup-call-exception-repeated-recovery-new-bus" and
            metadata.bus == reloaded_bus_name
      after
        80 ->
          false
  ...
  test "preserves cached lifecycle bus when bus_name lookup raises during refresh" do
    ...
    registry =
      start_supervised!(
        {LifecycleSubscriberTestRegistry,
         [skills: [], hook_defaults: %{}, bus_name: old_bus_name]}
      )
    ...
    assert_eventually(fn ->
      :ok =
        publish_lifecycle_signal(old_bus_name, "skill.pre", "/hooks/skill/pre", "before-raise")

      receive do
        {:telemetry, @telemetry_event, %{count: 1}, metadata} ->
          metadata.type == "skill.pre" and metadata.skill_name == "before-raise" and
            metadata.bus == old_bus_name
      after
        80 ->
          false
    ...
    assert_eventually(fn ->
      ...
      receive do
        {:telemetry, @telemetry_event, %{count: 1}, metadata} ->
          metadata.type == "skill.pre" and metadata.skill_name == "after-raise-old-bus" and
            metadata.bus == old_bus_name
      after
        80 ->
          false
  ...
  test "preserves cached lifecycle bus when bus_name lookup keeps exiting during refresh and migrates after recovery" do
    old_bus_name = "bus_#{System.unique_integer([:positive])}"
    reloaded_bus_name = "bus_#{System.unique_integer([:positive])}"
    ...
    lookup_plan =
      start_supervised!(
        {Agent,
         fn ->
           %{count: 0, fail_on: MapSet.new([6, 9]), fail_mode: {:exit, :bus_name_unavailable}}
         end}
      )
    ...
    assert_eventually(fn ->
      :ok =
        publish_lifecycle_signal(
          ...
          "skill.pre",
          "/hooks/skill/pre",
          "before-repeated-bus-name-exit-refresh"
        )

      receive do
        {:telemetry, @telemetry_event, %{count: 1}, metadata} ->
          metadata.type == "skill.pre" and
            metadata.skill_name == "before-repeated-bus-name-exit-refresh" and
            metadata.bus == old_bus_name
      after
        80 ->
          false
    ...
    assert_eventually(fn ->
      :ok =
        publish_lifecycle_signal(
          ...
          "skill.pre",
          "/hooks/skill/pre",
          "after-repeated-bus-name-exit-refresh-old-bus"
        )

      receive do
        {:telemetry, @telemetry_event, %{count: 1}, metadata} ->
          metadata.type == "skill.pre" and
            metadata.skill_name == "after-repeated-bus-name-exit-refresh-old-bus" and
            metadata.bus == old_bus_name
      after
        80 ->
          false
      ...
      fn ->
        :ok =
          publish_lifecycle_signal(
            ...
            "skill.pre",
            "/hooks/skill/pre",
            "after-repeated-bus-name-exit-refresh-new-bus"
          )
      end,
      6,
      80
      ...
      fn ->
        :ok =
          publish_lifecycle_signal(
            ...
            "skill.pre",
            "/hooks/skill/pre",
            "after-repeated-bus-name-exit-recovery-old-bus"
          )
      end,
      6,
      80
    ...
    assert_eventually(fn ->
      :ok =
        publish_lifecycle_signal(
          ...
          "skill.pre",
          "/hooks/skill/pre",
          "after-repeated-bus-name-exit-recovery-new-bus"
        )

      receive do
        {:telemetry, @telemetry_event, %{count: 1}, metadata} ->
          metadata.type == "skill.pre" and
            metadata.skill_name == "after-repeated-bus-name-exit-recovery-new-bus" and
            metadata.bus == reloaded_bus_name
      after
        80 ->
          false
  ...
  test "preserves existing lifecycle subscriptions when refresh fails adding new signal types" do
    ...
    registry =
      start_supervised!(
        {LifecycleSubscriberTestRegistry,
         [skills: [%{module: Jido.Code.Skill.Observability.TestSkills.ValidLifecycleHook}]]}
      )
    ...
    assert :ok =
             LifecycleSubscriberTestRegistry.set_skills(registry, [
               %{module: Jido.Code.Skill.Observability.TestSkills.InvalidLifecycleHook}
             ])
  ...
  test "starts with base subscriptions when registry hook paths are invalid and recovers after refresh" do
    ...
    registry =
      start_supervised!(
        {LifecycleSubscriberTestRegistry,
         [skills: [%{module: Jido.Code.Skill.Observability.TestSkills.InvalidLifecycleHook}]]}
      )
    ...
    assert :ok =
             LifecycleSubscriberTestRegistry.set_skills(registry, [
               %{module: Jido.Code.Skill.Observability.TestSkills.ValidLifecycleHook}
             ])
        ...
        {LifecycleSubscriberTestRegistry,
         [
           skills: [
             %{
               module:
                 Jido.Code.Skill.Observability.TestSkills.InheritGlobalSignalTypeLifecycleHook
             }
           ],
  ...
  test "starts without inherited lifecycle subscriptions when initial list_skills call keeps raising call exceptions across refreshes and recovers on refresh" do
    bus_name = "bus_#{System.unique_integer([:positive])}"
      ...
      fn ->
        :ok =
          publish_lifecycle_signal(
            ...
            "skill.default.custom.pre",
            "/hooks/skill/default/custom/pre",
            "before-list-skills-repeated-call-exception-recovery"
          )
      end,
      6,
      60
      ...
      fn ->
        :ok =
          publish_lifecycle_signal(
            ...
            "skill.default.custom.pre",
            "/hooks/skill/default/custom/pre",
            "after-first-list-skills-call-exception-refresh"
          )
      end,
      6,
      60
      ...
      fn ->
        :ok =
          publish_lifecycle_signal(
            ...
            "skill.default.custom.pre",
            "/hooks/skill/default/custom/pre",
            "after-second-list-skills-call-exception-refresh"
          )
      end,
      6,
      60
    ...
    assert_eventually(fn ->
      :ok =
        publish_lifecycle_signal(
          ...
          "skill.default.custom.pre",
          "/hooks/skill/default/custom/pre",
          "after-list-skills-repeated-call-exception-recovery"
        )

      receive do
        {:telemetry, @telemetry_event, %{count: 1}, metadata} ->
          metadata.type == "skill.default.custom.pre" and
            metadata.skill_name == "after-list-skills-repeated-call-exception-recovery"
      after
        80 ->
          false
      end
  ...
  test "starts without inherited lifecycle subscriptions when initial list_skills call keeps exiting across refreshes and recovers on refresh" do
    bus_name = "bus_#{System.unique_integer([:positive])}"
    registry_name = :"lifecycle_list_skills_exit_registry_#{System.unique_integer([:positive])}"
      ...
      fn ->
        :ok =
          publish_lifecycle_signal(
            ...
            "skill.default.custom.pre",
            "/hooks/skill/default/custom/pre",
            "before-list-skills-exit-repeated-recovery"
          )
      end,
      6,
      60
    ...
    Process.sleep(50)
    drain_telemetry_messages()
    assert :sys.get_state(subscriber).cached_hook_defaults == recovered_hook_defaults
      ...
      fn ->
        :ok =
          publish_lifecycle_signal(
            ...
            "skill.default.custom.pre",
            "/hooks/skill/default/custom/pre",
            "after-first-list-skills-exit-refresh"
          )
      end,
      6,
      60
      ...
      fn ->
        :ok =
          publish_lifecycle_signal(
            ...
            "skill.default.custom.pre",
            "/hooks/skill/default/custom/pre",
            "after-second-list-skills-exit-refresh"
          )
      end,
      6,
      60
    ...
    assert_eventually(fn ->
      try do
        LifecycleSubscriberTestRegistry.set_list_skills_error(registry_name, nil) == :ok
      catch
        :exit, _reason ->
          false
      end
    ...
    assert_eventually(fn ->
      :ok =
        publish_lifecycle_signal(
          ...
          "skill.default.custom.pre",
          "/hooks/skill/default/custom/pre",
          "after-list-skills-exit-repeated-recovery"
        )

      receive do
        {:telemetry, @telemetry_event, %{count: 1}, metadata} ->
          metadata.type == "skill.default.custom.pre" and
            metadata.skill_name == "after-list-skills-exit-repeated-recovery"
      after
        80 ->
          false
      end
  ...
  test "starts without inherited lifecycle subscriptions when initial hook defaults call keeps raising call exceptions across refreshes and recovers on refresh" do
    bus_name = "bus_#{System.unique_integer([:positive])}"
    ...
    lookup_plan =
      start_supervised!(
        {Agent, fn ->
          %{count: 0, fail_on: MapSet.new([1, 3, 5])}
        end}
      )
      ...
      fn ->
        :ok =
          publish_lifecycle_signal(
            ...
            "skill.default.custom.pre",
            "/hooks/skill/default/custom/pre",
            "before-repeated-startup-hook-default-call-exception-recovery"
          )
      end,
      6,
      60
      ...
      fn ->
        :ok =
          publish_lifecycle_signal(
            ...
            "skill.default.custom.pre",
            "/hooks/skill/default/custom/pre",
            "after-first-startup-hook-default-call-exception-refresh"
          )
      end,
      6,
      60
      ...
      fn ->
        :ok =
          publish_lifecycle_signal(
            ...
            "skill.default.custom.pre",
            "/hooks/skill/default/custom/pre",
            "after-second-startup-hook-default-call-exception-refresh"
          )
      end,
      6,
      60
    ...
    assert_eventually(fn ->
      :ok =
        publish_lifecycle_signal(
          ...
          "skill.default.custom.pre",
          "/hooks/skill/default/custom/pre",
          "after-repeated-startup-hook-default-call-exception-recovery"
        )

      receive do
        {:telemetry, @telemetry_event, %{count: 1}, metadata} ->
          metadata.type == "skill.default.custom.pre" and
            metadata.skill_name == "after-repeated-startup-hook-default-call-exception-recovery"
      after
        80 ->
          false
      end
  ...
  test "starts without inherited lifecycle subscriptions when initial hook defaults read keeps returning invalid data across refreshes and recovers on refresh" do
    bus_name = "bus_#{System.unique_integer([:positive])}"
      ...
      fn ->
        :ok =
          publish_lifecycle_signal(
            ...
            "skill.default.custom.pre",
            "/hooks/skill/default/custom/pre",
            "before-repeated-startup-hook-default-invalid-recovery"
          )
      end,
      6,
      60
      ...
      fn ->
        :ok =
          publish_lifecycle_signal(
            ...
            "skill.default.custom.pre",
            "/hooks/skill/default/custom/pre",
            "after-first-startup-hook-default-invalid-refresh"
          )
      end,
      6,
      60
      ...
      fn ->
        :ok =
          publish_lifecycle_signal(
            ...
            "skill.default.custom.pre",
            "/hooks/skill/default/custom/pre",
            "after-second-startup-hook-default-invalid-refresh"
          )
      end,
      6,
      60
    ...
    assert_eventually(fn ->
      :ok =
        publish_lifecycle_signal(
          ...
          "skill.default.custom.pre",
          "/hooks/skill/default/custom/pre",
          "after-repeated-startup-hook-default-invalid-recovery"
        )

      receive do
        {:telemetry, @telemetry_event, %{count: 1}, metadata} ->
          metadata.type == "skill.default.custom.pre" and
            metadata.skill_name == "after-repeated-startup-hook-default-invalid-recovery"
      after
        80 ->
          false
      end
  ...
  test "preserves inherited lifecycle subscriptions and cached hook defaults when list_skills call raises call exceptions during refresh" do
    ...
    assert :ok =
             LifecycleSubscriberTestRegistry.set_skills(registry, [
               %{module: Jido.Code.Skill.Observability.TestSkills.ExplicitLifecycleHook}
             ])
  ...
  test "refreshes explicit hooks while preserving inherited disable when hook defaults refresh fails" do
    ...
    registry =
      start_supervised!(
        {LifecycleSubscriberTestRegistry,
         [
           skills: [%{module: Jido.Code.Skill.Observability.TestSkills.InheritedLifecycleHook}],
           hook_defaults: %{
             pre: %{enabled: false, signal_type: "skill/pre"},
             post: %{enabled: true, signal_type: "skill/post"}
           }
    ...
    assert :ok =
             LifecycleSubscriberTestRegistry.set_skills(registry, [
               %{module: Jido.Code.Skill.Observability.TestSkills.InheritedLifecycleHook},
               %{module: Jido.Code.Skill.Observability.TestSkills.ExplicitLifecycleHook}
             ])
  ...
  defp write_skill(root, dir_name, skill_name, route, opts \\ []) do
    ...
    allowed_tools_line =
      if is_nil(allowed_tools), do: "", else: "allowed-tools: #{allowed_tools}\n"
    ...
    jido:
      actions:
        - Jido.Code.Skill.Observability.TestActions.HookSignal
      router:
        - "#{route}": HookSignal
    #{hooks_block}---
