...
| `skill/permission/blocked` | Signal dispatcher | `skill_name`, `route`, `reason`, `tools`, `timestamp` | `/permissions/{signal_type}` |
...
4. `reason` in `skill/permission/blocked` is `ask` or `denied`; `tools` is the matched tool list.
